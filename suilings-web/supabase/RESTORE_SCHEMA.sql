-- ============================================================
-- SUILINGS FULL SCHEMA (BACKUP / RESTORE)
-- Run this in Supabase SQL Editor to create all tables used by the app.
-- Use after disaster recovery or when redeploying to a fresh database.
-- Creates only: profiles, playground_snippets, exercise_progress,
-- exercises, feedback, sbt_credentials, user_wallets, activity_feed.
-- ============================================================
...
-- Atomic increment functions (used by app RPC)
CREATE OR REPLACE FUNCTION increment_view_count(snippet_id UUID)
RETURNS VOID AS $$
BEGIN
  UPDATE playground_snippets
...
-- Triggers: write to activity_feed on exercise completion and snippet creation
...
BEGIN
  IF NEW.completed = true AND (OLD.completed IS NULL OR OLD.completed = false) THEN
    ...
    VALUES (
      NEW.user_id,
      'exercise_completed',
      jsonb_build_object('exercise_id', NEW.exercise_id, 'exercise_name', NEW.exercise_id)
    );
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER on_exercise_completed
  AFTER UPDATE ON exercise_progress
  FOR EACH ROW
  EXECUTE FUNCTION create_exercise_completion_activity();
...
BEGIN
  ...
  VALUES (
    NEW.user_id,
    'snippet_created',
    jsonb_build_object('snippet_id', NEW.id, 'snippet_title', NEW.title, 'is_public', NEW.is_public)
  );
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER on_snippet_created
  AFTER INSERT ON playground_snippets
  FOR EACH ROW
  WHEN (NEW.user_id IS NOT NULL)
  EXECUTE FUNCTION create_snippet_activity();

-- ---------------------------------------------------------------------------
-- RPC: get_credential_by_identifier (for /api/credential/verify/[identifier])
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT
    ...
    c.github_username,
    c.wallet_address,
    c.completed_exercises,
    c.completion_date,
    c.streak_days,
    c.total_time_minutes,
    c.mint_transaction_digest,
    c.mint_status,
...
-- ---------------------------------------------------------------------------
-- Done. 8 tables + RPC. Safe to run multiple times (IF NOT EXISTS / DROP IF EXISTS).
-- ---------------------------------------------------------------------------
