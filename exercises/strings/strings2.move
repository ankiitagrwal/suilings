// Exercise: Advanced String Operations
//
// Implement advanced string manipulation.
//
...
module suilings::strings2_tests {
    ...
    assert!(sub == string::utf8(b"Hello"));

    let s2 = string::utf8(b"Move Language");
    let sub2 = strings2::substring(s2, 5, 13);
    assert!(sub2 == string::utf8(b"Language"));
}

#[test]
    fun contains() {
        let s = string::utf8(b"Hello, World!");
        assert!(strings2::contains(s, string::utf8(b"World")));
        assert!(!strings2::contains(string::utf8(b"Hello, World!"), string::utf8(b"Rust")));
}

    #[test]
    fun starts_with() {
        let s = string::utf8(b"Hello, World!");
        assert!(strings2::starts_with(s, string::utf8(b"Hello")));
        assert!(!strings2::starts_with(string::utf8(b"Hello, World!"), string::utf8(b"World")));
}

    #[test]
    fun ends_with() {
        let s = string::utf8(b"Hello, World!");
        assert!(strings2::ends_with(s, string::utf8(b"World!")));
        assert!(!strings2::ends_with(string::utf8(b"Hello, World!"), string::utf8(b"Hello")));
}

    #[test]
    fun count_char() {
        let s = string::utf8(b"hello");
        assert!(strings2::count_char(s) == 2); // 'l' = 108

        let s2 = string::utf8(b"Mississippi");
        assert!(strings2::count_char(s2) == 4); // 's' = 115
        assert!(strings2::count_char(string::utf8(b"Mississippi"), 105) == 4); // 'i' = 105
}

    #[test]
    fun to_uppercase_ascii() {
        let s = string::utf8(b"hello");
        let upper = strings2::to_uppercase_ascii(s);
        assert!(upper == string::utf8(b"HELLO"));

        let s2 = string::utf8(b"Hello World!");
        let upper2 = strings2::to_uppercase_ascii(s2);
        assert!(upper2 == string::utf8(b"HELLO WORLD!"));
}
