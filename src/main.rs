...
/// Suilings is a collection of small exercises to get you used to writing Move code for the Sui blockchain
struct Args {
...
/// Lists the exercises available in Suilings
struct ListArgs {
    #[argh(switch, short = 'p')]
...
fn main() {
    ...
    if !Path::new("info.toml").exists() {
        println!(
            "{} must be run from the Suilings directory",
            std::env::current_exe().unwrap().to_str().unwrap()
        );
        println!("Try `cd Suilings/`!");
        std::process::exit(1);
    }

    if !check_sui_installation() {
        println!("We cannot find `sui`.");
        println!("Please install the Sui CLI first.");
        ...
        Subcommands::Watch(subargs) => {
            ...
            match watching {
                ...
                Ok(WatchStatus::Unfinished) => {
                    println!("We hope you're enjoying learning about Move and the Sui blockchain!");
                    println!("If you want to continue working on the exercises later, you can simply run `suilings watch` again");
                }
            }
        }
    }
...
fn check_sui_installation() -> bool {
             ...
             Ok(event) => match event.kind {
                EventKind::Create(_) | EventKind::Modify(_) => {
                    for path in event.paths {
                        if path.extension() == Some(OsStr::new("move")) && path.exists() {
                            let filepath = path.canonicalize().unwrap();
                            let pending_exercises = exercises
                                .iter()
                                .find(|e| filepath.starts_with(&e.path))
                                .into_iter()
                                .chain(
                                    exercises
                                        .iter()
                                        .filter(|e| !e.looks_done() && !filepath.starts_with(&e.path)),
                                );
...
const DEFAULT_OUT: &str = r#"Thanks for installing Suilings!

Is this your first time? Don't worry, Suilings is made for beginners! We are
going to teach you a bunch of stuff about Move and the Sui blockchain.

Here's how Suilings works:

1. To start Suilings run `cargo run -r --bin suilings watch`
2. It'll automatically start with the first exercise. Don't get confused by
   error messages popping up as soon as you run Suilings! This is part of the
...
We hope you enjoyed learning about Move and the Sui blockchain!
If you noticed any issues, please don't hesitate to report them."#;

const WELCOME: &str = r#"Suilings - An interactive tutorial to get started with Move on Sui

 ____  _   _ ___ _     ___ _   _  ____ ____  
/ ___|| | | |_ _| |   |_ _| \ | |/ ___/ ___| 
\___ \| | | || || |    | ||  \| | |  _\___ \ 
 ___) | |_| || || |___ | || |\  | |_| |___) |
|____/ \___/|___|_____|___|_| \_|\____|____/ 
                                            "#;
